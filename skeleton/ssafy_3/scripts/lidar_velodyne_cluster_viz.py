...
class Cluster_viz:
    ...
    def __init__(self):
        ...
        rospy.Subscriber("/clusters", PoseArray, self.callback) # header, Pose poses[]
        '''
        ...
        while not rospy.is_shutdown():
            if self.is_odom == True and self.cluster_status == True:
                ...
                #TODO: (1) 좌표 변환 행렬 생성 -> [R|T] only for orientation Yaw and coordi X,Y
                '''
                trans_matrix = np.array([
                                            [       ,       ,       ],
                                            [       ,       ,       ],
                                            [0,0,1]])
                ex) np.array([[cosY, -sinY, coordi_X],
                      [sinY, cosY, coordi_Y], 
                      [0, 0, 1]])

                '''
                trans_matrix = np.array([
                    [np.cos(self.vehicle_yaw), -np.sin(self.vehicle_yaw), self.vehicle_pos_x],
                    [np.sin(self.vehicle_yaw), np.cos(self.vehicle_yaw), self.vehicle_pos_y],
                    [0, 0, 1]
                ])
                obj_data=PointCloud()
                '''
                ...
                for num,i in enumerate(self.cluster_data.poses) : # Pose : x,y / enumerate -> create tuple (0,result),(1,result),(2,result)...
                    # print('num',num)
                    # print('i',i) # pose : x, y, 1 / orientation : x, y, z, w
                    
                    #TODO: (2) PointCloud와 ObjectStatus 형식에 맞춘 메시지 데이터 생
    ...
    def odom_callback(self,msg): 
        ...
        self.vehicle_pos_x=msg.pose.pose.position.x # map coordi
