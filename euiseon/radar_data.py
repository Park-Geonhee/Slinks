...
sensorPosition = np.array([parameters_radar.get(i) for i in (["X","Y","Z"])]) 
sensorRPY = np.array([parameters_radar.get(i) for i in (["ROLL","PITCH","YAW"])])  
...
class Radar :
    ...
    def scan_callback(self, msg): #RaderDetection[] RadarDetections
        ...
        print("time stamp",time.strftime('%Y-%m-%d %H:%M:%S'))
