...
class Radar :
    ...
    def right_callback(self, msg): #RaderDetection[] RadarDetections
        ...
        for point in msg.detections :
            ...
            print(point.detection_id,np.round(trans_data[0],4).astype(float), np.round(trans_data[1],4).astype(float), np.round(trans_data[2],4).astype(float),  int(point.rangerate))
