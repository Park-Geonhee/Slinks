...
#         img_ylane = cv2.inRange(img_hsv, self.lower_ylane, self.upper_ylane)
        ...
        img_ylane = cv2.inRange(img, self.lower_rlane, self.upper_rlane)
...
class CURVEFit:
    def __init__(self, order, lane_width, y_margin, x_range, dx, min_pts,alpha):
        ...
        # self.ransac_right = linear_model.RANSACRegressor(base_estimator=linear_mode
        # l.LassoCV(alphas=[1,0.1,0.01,0.5],cv=5),
        #                                                  max_trials=10000000000000,
        #                                                  loss='absolute_loss',
        #                                                  min_samples=self.min_pts,
        #                                                  residual_threshold=self.y_margin)
